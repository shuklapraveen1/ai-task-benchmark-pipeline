...
- **Container build context does not exclude this pipeline's own source directory.** While reviewing a passing - **`glom` containerization now passes deterministically across two consecutive runs** (`379/379` tests, `regression_detected: False`, `accepted: True` both times) using a scoped `pytest -q` command. Note: this run did not include `--doctest-modules`, so it does not by itself confirm the `tutorial.py` doctest-ordering issue from REPORT.md §8.1 is resolved — see §8.1/§11 for that specific finding. The earlier `pytest 6.2.5` vs `pytest 9.1.1` collection-order concern still applies if the doctest-inclusive command is used.
